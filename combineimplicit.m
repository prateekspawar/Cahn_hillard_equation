...
for i=1:N-1
    if(i==1)
        up(i)=(-A/r(1)/(2*h)*(1-2*chi*ck(1)*(1-ck(1)))+2*chi*2*(1-2*ck(1))/(2*h)*(ck(2)-ck_l)/(2*h) ...
       -(1-2*chi*ck(1)*(1-ck(1)))/(h*h));
    else
        up(i)=(-A/r(i)/(2*h)*(1-2*chi*ck(i)*(1-ck(i)))+2*chi*2*(1-2*ck(i))/(2*h)*(ck(i+1)-ck(i-1))/(2*h) ...
       -(1-2*chi*ck(i)*(1-ck(i)))/(h*h));
    end
end
for i=2:N
    if(i==N)
       down(i)=(-A/r(i)*(1-2*chi*ck(i)*(1-ck(i)))*(-1/2/h)+2*chi*(1-2*ck(i))/(h)*(-1/2/h) ...
